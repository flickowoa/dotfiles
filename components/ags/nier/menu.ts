// shared nier menu styling for the popups + the top-bar frames. light/dark aware.
// the main interaction is the "rail tab" (like the in-game map menu tabs):
//   idle   = faint fill, rails hidden
//   hover  = tinted fill + the top/bottom bars fade in, inset from the corners
//   active = full fill, inverted text, rails solid when the modal is open
...
// "r,g,b" (0-255) -> [r,g,b] floats for cairo
...
// fg / bg rgb triplets for the current theme
...
// ── RailTab — the rail-tab widget ───────────────────────────────────────────
// returns { box, refresh, setActive } and reacts to theme toggles on its own.
// every frame shares a fixed inner height so they all line up regardless of font
// size (body height + the two rails + the two gaps -> FRAME_TOTAL_H).
const RAIL_H = 3
const GAP_H  = 3
export const FRAME_BODY_H = 32
export const FRAME_TOTAL_H = FRAME_BODY_H + (RAIL_H + GAP_H) * 2   // 44
// active frames grow a bit and drop down so their bottom edge meets the separator
...
export const RailTab = ({
    ...
    // layout: [ topRail | topGap | body | bottomGap | bottomRail ]. the gaps are
    // spacers we animate to nudge the body down on active while keeping the total
    // height constant - so the bar never jumps.
    ...
    // optional leading square - a DrawingArea, not a css box (those stretched on
// me idk). fixed size_request so its an actual square, coloured in apply() to make it similar to the game ui.
    ...
    // force the body height (css min-height wasnt reliable on a box)
    ...
    const apply = () => {
        ...
        // hover + active both invert (solid fill, bg-coloured square + text). the
        // difference: hover shows the rails, active hides them and nudges the whole (small details to make look like tha game)
        // frame down a few px (animated).
        ...
        // active grows the body to ACTIVE_BODY_H, height animated via min-height
        ...
        `
        // square colour follows the text - fg idle, inverted bg when selected
        if (sq) { sqRGB = sel ? toRGB(p.bg) : toRGB(p.fg); sq.queue_draw() }
        // rails only show on hover, same css so same width, inset 3px to leave a
        // tiny corner gap. active hides both and collapses the bottom one so the
        // body can reach down to the separator.
        ...
        `
        // gaps keep the total height fixed. active: bottom gap -> 0 and the top
        // takes the rest so the bigger body sits flush. otherwise theyre symmetric.
        ...
        setup: (self: any) => {
            ...
            if (onSingle || onDouble || onButton) {
                self.connect("button-press-event", (_w: any, ev: any) => {
                    // had to use get_button() here - reading ev.button on a gdk event
                    // gives the EventButton struct back, not the number, so clicks
                    // werent routing right. get_button() returns [ok, buttonNumber].
                    ...
                    if (single) {
                        ...
                        if (onDouble) {
                            // short delay to tell single vs double click apart
...
// thin wrapper for the older popup callers (wifi, calendar)
